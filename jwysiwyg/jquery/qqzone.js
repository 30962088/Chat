...
F4A.nameSpace('controls.CommentBox.extensions', new function () {
    var Control = F4A.controls.Base;
    var Template = F4A.controls.Template;
    ...
    function saveLocalData(key, value) {
        try {
            ...
            return so.set(key, value);
        } catch (e) {
            return QZONE.FP._t.QZFL.dataCenter.save(key, value);
        }
    }
    this.CheckBox = Control.derive(new function () {
        this.constructor = function () {
            if (Control.apply(this, arguments)) {}
        };
        this.extendPrototype = function () {
            this.className = 'PresentInserter';
            this.version = '1.0';
            this.genProperties({
                target: {},
                isChecked: {
                    onChanged: function (original, isChecked) {
                        this._updateUIforIsChecked(isChecked);
                    }
                },
                text: {
                    onChanged: function (original, text) {
                        this._updateUIforText(text);
                    }
                },
                localKey: {}
            });
            this.onRootVisualChanged = function () {
                this._initUIforIsChecked();
                this._initUIforText();
            };
            ...
            this._initUIforIsChecked = function () {
                var $checkBox = this;
                ...
                if ($_checkBox) {
                    $_checkBox.onclick = function () {
                        ...
                        var key = $checkBox.getLocalKey();
                        if (key) {
                            saveLocalData(key, (checked ? 1 : 0));
                        }
                        setTimeout(function () {
                            $checkBox.setIsChecked(checked);
                            $checkBox.getTarget().focus();
                        });
                        return false;
                    };
                    ...
                    $checkBox._updateUIforIsChecked($checkBox.setIsChecked($_checkBox.checked));
                }
            };
            this._updateUIforIsChecked = function (isChecked) {
                var $_checkBox = this.$('checkBox');
                if ($_checkBox) {
                    $_checkBox.checked = !! isChecked;
                }
            };
            this._initUIforText = function () {
                this._updateUIforText(this.getText());
            };
            this._updateUIforText = function (text) {
                var $_text = this.$('text');
                if ($_text) {
                    $_text.innerHTML = escHTML(text);
                }
            };
            this.setTemplate(Template.parse(['<span class="add_right"><input type="checkbox" x:id="checkBox" /><label x:id="text" x:for="checkbox"></label></span>'].join('')));
        };
        this.extendClass = function () {};
    })
});
F4A.nameSpace('controls.CommentBox.extensions', new function () {
    ...
    var ItemsControl = $fc.ItemsControl;
    var Template = $fc.Template;
    var emoticonSrcPattern = /^[\w\.\/:]*\/qzone\/em\/e(\d+)\.gif$/g;
    this.EmoticonInserter = Control.derive(new function () {
        this.constructor = function () {
            if (Control.apply(this, arguments)) {}
        };
        this.extendPrototype = function () {
            this.className = 'EmoticonInserter';
            this.genProperties({
                target: {}
            });
            this.dispose = function () {
            ...
            this.onRootVisualChanged = function ($_original, $_root) {
                if ($_original) {
                    QZFL.event.removeEvent($_original, 'click', this.$_root_onClick);
                }
                if ($_root) {
                    QZFL.event.addEvent($_root, 'click', this.$_root_onClick || (this.$_root_onClick = this.createHandler(function (e) {
                        F4A.FP.showMsgbox('正在加载表情选择器...', 6, -1);
                        QZFL.event.removeEvent(this.getRootVisual(), 'click', this.$_root_onClick);
                        var bindEmoticonPanel = this.createHandler(function () {
                            F4A.FP.hideMsgbox();
                            var $target = this.getTarget();
                            var $emoticonPanel = QZFL.widget.emotion.bind(this.getRootVisual());
                            $emoticonPanel.onShow = this.createHandler(function (panel_, button_) {
                                return this.onShow && this.onShow(panel_, button_);
                            });
                            $emoticonPanel.onSelect = function (o) {
                                var $_emoticon = new Image;
                                $_emoticon.tabindex = -1;
                                $_emoticon.unselectable = true;
                                $_emoticon.className = 'emoticon';
                                ua.ie ? $_emoticon.useMap = parseInt(o.id) + 100 : $_emoticon.setAttribute('name', parseInt(o.id) + 100);
                                $_emoticon.src = ['http://', F4A.FP.getBase().siDomain, '/qzone/em/e', ua.ie ? $_emoticon.useMap : $_emoticon.getAttribute('name'), '.gif'].join('');
                                $_emoticon.alt = '☺';
                                var $content = $target.getContentBox();
                                $content.addListener('onFocus', function () {
                                    $content.removeListener('onFocus', arguments.callee);
                                    $content.insertToContent($_emoticon, 'node');
                                });
                                $content.focus();
                            };
                            $emoticonPanel.show();
                        });
                        if (QZFL.widget.emotion) {
                            bindEmoticonPanel();
                        } else {
                            var url = ['http://', F4A.FP.getBase().siDomain, '/ac/qzfl/release/widget/emotionselector.js'].join('');
                            var jsLoader = new QZFL.JsLoader();
                            jsLoader.onload = bindEmoticonPanel;
                            jsLoader.load(url, null, 'utf-8');
                            jsLoader = null;
                        }
                        bindEmoticonPanel = null;
                    })));
                }
                $_original = $_root = null;
            };
            this.setTemplate(Template.parse(['<a href="javascript:void(0);" class="add_emoticon c_tx3" title="插入表情"><i class="ui_ico"></i>表情</a>'].join('')));
        };
        this.extendClass = function () {};
    });
});
F4A.nameSpace('controls.CommentBox.extensions', new function () {
    ...
    var ItemsControl = $fc.ItemsControl;
    var Template = $fc.Template;
    this.PresentInserter = Control.derive(new function () {
        this.constructor = function () {
            if (Control.apply(this, arguments)) {}
        };
        this.extendPrototype = function () {
            ...
            this.onRootVisualChanged = function ($_original, $_root) {
                var $pi = this;
                if ($_original) {
                    QZFL.event.removeEvent($_original, 'click', $pi.$_root_onClick);
                }
                if ($_root) {
                    QZFL.event.addEvent($_root, 'click', $pi.$_root_onClick || ($pi.$_root_onClick = function (e) {
                        F4A.FP.popupDialog('送礼物', {
                            src: ['http://', F4A.FP.getBase().imgcacheDomain, '/qzone/gift/send_list.html?type=936&uin=', $pi.getTargetUin()].join('')
                        }, 625, 532, false);
                        F4A.FP.appendPopupFn(function () {
                            $pi.getTarget().focus();
                            $pi.onPresentSelected();
                        });
                        F4A.FP.sendPV('mall.qzone.qq.com', 'isd.qzonegift.shuoshuo-shuoshuo.entrance');
                    }));
                }
                $_original = $_root = null;
            };
            this.onPresentSelected = function () {};
            this.setTemplate(Template.parse(['<a href="javascript:void(0);" class="add_gift c_tx3" title="选择礼物"><i class="ui_ico"></i>礼物</a>'].join('')));
        };
        this.extendClass = function () {};
    });
});
F4A.nameSpace('controls.CommentBox.extensions', new function () {
    ...
    var ItemsControl = $fc.ItemsControl;
    var Template = $fc.Template;
    this.MentionInserter = Control.derive(new function () {
        this.extendPrototype = function () {
            this.className = 'MentionInserter';
            this.genProperties({
                target: {}
            });
            this.dispose = function () {
            ...
            this.onRootVisualChanged = function (_original, _root) {
                ...
                if (_root) {
                    QZFL.event.addEvent(_root, 'click', this._root_onClick || (this._root_onClick = this.createHandler(function (e) {
                        ...
                        var target$ = mentionInserter$.getTarget();
                        target$.doNotCollapse = true;
                        var base = F4A.FP.getBase();
                        base.popupCallback = function (popupFrame) {
                            base.popupCallback = null;
                            popupFrame.callback = function () {
                                var friendsSelector = popupFrame.contentWindow.QZONE.friends.Selector;
                                friendsSelector.setup({
                                    certified: 1,
                                    uin: F4A.FP.getLoginUin(),
                                    multiChoose: false,
                                    module: '000',
                                    row: 3,
                                    result: false,
                                    ...
                                    selectCallback: function () {
                                        ...
                                        var contentBox$ = target$.getContentBox();
                                        var loader = SensibleEditor.friendSelectorLoader;
                                        loader.call(loader, contentBox$, function (selector) {
                                            var FriendSelector = SensibleEditor.responsors.FriendSelector;
                                            var selectedFriends = friendsSelector.getSelect();
                                            for (var i in selectedFriends) {
                                                var selectedFriend = selectedFriends[i];
                                                selectedFriend = {
                                                    id: selectedFriend.uin,
                                                    name: restHTML(selectedFriend.name || ''),
                                                    memo: restHTML(selectedFriend.remark || '')
                                                };
                                                contentBox$.addListener('onFocus', function () {
                                                    contentBox$.removeListener('onFocus', arguments.callee);
                                                    contentBox$.insertToContent(FriendSelector.createMention({
                                                        getMemo: function () {
                                                            return selectedFriend.memo;
                                                        },
                                                        getUin: function () {
                                                    ...
                                                    selector.onFriendSelected(selectedFriend);
                                                    mentionInserter$.onFriendSelected();
                                                    F4A.FP.closePopup();
                                                });
                                                contentBox$.focus();
                                                break;
                        ...
                        F4A.FP.appendPopupFn(function () {
                            target$.focus();
                            delete target$.doNotCollapse;
                        });
                        F4A.FP.popupDialog('选择用户', {
                            src: ['http://', F4A.FP.getBase().imgcacheDomain, '/qzone/app/mood/friendsSelector.html'].join('')
                        }, 500, 350);
                        return false;
                    })));
                }
            ...
            this.onFriendSelected = function () {};
            this.setTemplate(Template.parse(['<a href="javascript:void(0);" class="add_at c_tx3" title="选择用户"><i class="ui_ico"></i>用户</a>'].join('')));
        };
    });
});
/* |xGv00|3bec8da0321a031643d7718991204c42 */